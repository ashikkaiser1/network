...
class CI_DB_active_record extends CI_DB_driver {
     ...
     * @return object
     */
    function insert_on_duplicate_update_batch($table = '', $set = NULL, $primeryKey = '') {
        if (!is_null($set)) {
            $this->set_insert_batch($set);
        }
...
//echo count($this->ar_set);
        // Batch this baby 
        for ($i = 0, $total = count($this->ar_set); $i < $total; $i = $i + 100) {

            $sql = $this->_insert_on_duplicate_update_batch($this->_protect_identifiers($table, TRUE, NULL, FALSE), $this->ar_keys, array_slice($this->ar_set, $i, 100), $primeryKey);
