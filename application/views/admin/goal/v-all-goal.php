        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr ng-repeat="goals in all_goals| filter :name " id="tr{{goals.offer_goal_id}}">
                    ...
                    <td> {{goals.payout_cost}}</td>
                    <td> {{goals.RevenueTypeName}}</td>
                    <td> {{goals.revenue_cost}}</td>

                    <td>
                        <a type="button" href="<?php echo SITEURL . "admin/goals/UpdateGoals/" ?>{{goals.campaign_id}}/{{goals.offer_goal_id}} " class="btn btn-primary waves-effect waves-light m-b-5 btn-xs"><span class="fa fa-edit"></span></a>
