...
class campaign extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->load->model("affiliate/m_creative");
        $this->load->model("affiliate/m_goals");
    ...
    public function index() {
        ...
        $data['PageContent'] = $this->load->view("affiliate/campaign/v-campaign", $data, TRUE);
        $this->load->view("affiliate/template", $data);
    }
    public function offer_gen (){
        $data = array();
        $data['PageContent'] = $this->load->view("affiliate/campaign/offergen", $data, TRUE);
        $this->load->view("affiliate/template", $data);
	}
...
//        $data['PageContent'] = 	$this->load->view("affiliate/campaign/v-offer-deisgn-2", $data, TRUE);
        $data['PageContent'] = $this->load->view("affiliate/campaign/all_offer_tabular_view", $data, TRUE);
        $this->load->view("affiliate/template", $data);
    }
...
//        echo '<pre>';
        ...
        if (!$this->check_offer_approve($campaign_id, UID)) {
            ...
            $data['trackingPanel'] = $this->load->view("affiliate/offer/offer_tracking_link", $data, TRUE);

            $data['p_type'] = array("0" => "Postback Url", "1" => "Image Pixel", "2" => "Iframe Pixel");
            $data['PixelManager'] = $this->load->view("affiliate/offer/v-pixelManager", $data, TRUE);
        } else if ($this->m_campaign->is_pending_request($campaign_id, UID)) {
            //Request is pending and not approved;
            $data['trackingPanel'] = "Your Request is pending";
        } else {
            $data['trackingPanel'] = $this->load->view("affiliate/offer/offer_request_form", $data, TRUE);
        }

        $data['payout'] = $this->load->view("affiliate/offer/offer_payout", $data, TRUE);

        $data['getTargeting'] = $this->load->view("affiliate/offer/offer_geo", $data, TRUE);
        ...
        $data['creativePanel'] = $this->load->view("affiliate/offer/offer_creative", $data, TRUE);


        ///offer Landing Pages
        $data['OfferUrls'] = $this->load->view("affiliate/offer/offer_url/all-offer-urls", $data, TRUE);
        //end



        $data['offer_goals'] = $this->m_goals->getGoals($filterCreavtive);
        $data['goals'] = $this->load->view("affiliate/offer/offer_goals", $data, TRUE);


//        print_r($data);
//        die();
        $data['PageContent'] = $this->load->view("affiliate/offer/offer-details", $data, TRUE);
        $this->load->view("affiliate/template", $data);
    }

    public function getCustomPayouts() {
        ...
        if ($request) {
            ...
            $json['payouts'] = $this->m_campaign->get_custom_payouts($request);
            if (empty($json['payouts'])) {
                $json['payouts'] = $this->m_campaign->get_group_payout($request);
                if (!empty($json['payouts'])) {
                    $json['success'] = TRUE;
                }
            } else {
...
//                " . $checkData['short_url']."
    ...
    public function setConversionPixelUrl() {
        ...
        //function is used by affiliate/publisher
...
//        print_r($request);
    ...
    public function featureoffer() {
        //get featured offers
        $this->load->model("affiliate/m_featured_offers");
        $filter = array();
