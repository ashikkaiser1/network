...
class offer_urls extends CI_Controller {
    ...
    public function AddOfferLandingUrl($campaign_id = 0) {
        ...
        $camp = array();
        if($campaign_id !=0){
            $filter['campaign_id']= array($campaign_id);
        ...
        $data['FormAction'] = SITEURL . "admin/offer_urls/AddOfferLandingUrl";
        ...
        $data['PageContent'] = $this->load->view("admin/offer/offer_url/add-offer-url.php", $data, TRUE);
...
//        die();
        ...
        if(!empty($data['offerUrlData'])){
            $filter['campaign_id'] = $data['offerUrlData'];
            $camp = $this->m_campaign->getCampaign($filter);
        }
       
        $data['camapign'] = $camp;

        $data['FormAction'] = SITEURL . "admin/offer_urls/UpdateOfferUrl/" . $url_id;
