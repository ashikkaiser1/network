...
class offer extends CI_Controller {
    ...
    public function offer_gen (){
        //set ou uload new creatives to offers  
        $data = array();
        

        $data['PageContent'] = $this->load->view("admin/offer/offergen", $data, TRUE);
...
//            $campaign['end_date'] = isset($request['end_date']) ? $request['end_date'] : '';
            ...
            //cmp_id id third party network offer id
            $campaign['cmp_id'] = isset($request['cmp_id']) ? $request['cmp_id'] : 0;
            ...
            $campaign['all_os'] = isset($request['Alloffer_OS']) || empty($request['Alloffer_OS']) ? 1 : 0;
            $campaign['all_devices'] = isset($request['Alloffer_devices']) || empty($request['Alloffer_devices']) ? 1 : 0;
...
//            print_r($campaign);
            ...
            if ($campaign_id = $this->m_campaign->CreateCampaign($campaign)) {
                ...
                if (!(isset($request['Alloffer_devices']) && $request['Alloffer_devices'] == '1')) {
                ...
                //
                //start set offerOs
                if (!(isset($request['Alloffer_OS']) && $request['Alloffer_OS'] == '1')) {
...
//            print_r($post);
                ...
                //$json['redirect'] = SITEURL . "admin/campaign/offerRequest/" . $campaign_id;
                $json['redirect'] = SITEURL . "admin/offer/SetOfferCreative/" . $campaign_id;
        ...
        $data['FormAction'] = SITEURL . "admin/offer/CreateOffers";
...
//                $this->m_campaign->getCampaign($filter);
    ...
    public function setUpCampaignToCap($campaign_id = 0) {
        ...
        if ($request && $campaign_id) {
            ...
            if (isset($request['daily_cap']) && $request['daily_cap'] != '') {
            ...
            if (isset($request['monthly_cap']) && $request['monthly_cap'] != '') {
                $CampToCap[] = array("campaign_id" => $campaign_id,
            ...
            $this->m_campaign->setCampaignToCaps($CampToCap, $campaign_id);
        }
    }
...
//        delete or remove the post from campaign that is attach to it
    ...
    public function show_campaign() {
        ...
        $data['stats_link'] = SITEURL . "admin/report/advance_report?repType=offers_report";
...
//                    "category_id" => $row['category_id']
    ...
    public function bulkupdate() {
        ...
        if ($getRequest) {
            ...
            $offer_ids = isset($request['campaign_id']) ? $request['campaign_id'] : 0;
            unset($request['campaign_id']);

            $CampaignData = array();
            $CampaignData['status'] = $request['status'];

            if ($this->m_campaign->UpdateCampaign($CampaignData, $offer_ids)) {
                $json['success'] = TRUE;
                $json['msg'] = "Update Complete";

                $this->send_bulk_notification_to_affiliate($offer_ids);
//                //Kuldeep singh
            } else {
                $json['success'] = FALSE;
    ...
    public function send_bulk_notification_to_affiliate($offer_ids = array()) {
        $offers = $this->m_campaign->getCampaign(array("campaign_id" => $offer_ids, "group_by" => "campaign_id"));
        if (!empty($offers)) {

            $pub_filter = array();
            $pub_filter['UTID'] = AFFILIATE;
            $users = $this->m_users->getUsers($pub_filter);
            foreach ($offers as $one_offer) {
                $this->send_notification_to_affiliate($one_offer, $users);
            }
        }
    }
    ...
    public function UpdateOfferPayout($campaign_id = 0) {
        ...
        if (!empty($data['campToCaps'])) {
            ...
            foreach ($data['campToCaps'] as $row) {
                $capList[$row['capType']] = $row;
            }
            $data['campToCaps'] = $capList;
            $data['campaign']['daily_cap'] = isset($capList[1]) ? $capList[1]['cap'] : '';
            $data['campaign']['daily_budget'] = isset($capList[1]) ? $capList[1]['budget'] : '';

            $data['campaign']['monthly_cap'] = isset($capList[2]) ? $capList[2]['cap'] : '';
            $data['campaign']['monthly_budget'] = isset($capList[2]) ? $capList[2]['budget'] : '';
        }
//        echo '<pre>';
//        print_r($data['campToCaps']);
        ...
        $data['FormAction'] = SITEURL . "admin/offer/UpdateOfferPayout/" . $campaign_id;
        $data['SubmitBtn'] = "Update";
        $data['Submiting'] = "Update...";
        $data['title'] = "Update Offer Payout";

        $data['PageContent'] = $this->load->view("admin/offer/offer_edit/payout_edit", $data, TRUE);
        $this->load->view("admin/template", $data);
    }
...
//            die();
            ...
            if ($this->m_campaign->UpdateCampaign($campaign, $campaign_id)) {
                ...
                $this->send_notification_to_affiliate($request);
            } else {
                $json['success'] = FALSE;
...
//        print_r($data['post']);
        ...
        $data['FormAction'] = SITEURL . "admin/offer/UpdateOfferDetails/" . $campaign_id;
        $data['SubmitBtn'] = "Update";
        $data['Submiting'] = "Updating...";
        ...
        // $data['Campaign'] = $this->m_campaign->getCategory(0,array("allchild"=>true));
        //$data['allCategory'] = $this->load->view("admin/Campaign/all-Campaign", $data, TRUE);
        $data['PageContent'] = $this->load->view("admin/offer/offer_edit/offer_details", $data, TRUE);
        $this->load->view("admin/template", $data);
    }
    ...
    public function send_notification_to_affiliate($campaign = array(), $users = array()) {
        //Send notification to Affiliate when a certain offer change its 
        //status
        if ($campaign) {
            ...
            foreach ($users as $user) {
                ...
                $status = isset($campaign['c_status']) ? $campaign['c_status'] : $campaign['status'];

                switch ($status) {
                    case 0: $notification['title'] = "Offer ID : {$campaign['campaign_id']} is  In-Activate.";
                        break;
                    case 1: $notification['title'] = "Offer ID : {$campaign['campaign_id']} is  Activated.";
                        break;
                    case 2: $notification['title'] = "Offer ID : {$campaign['campaign_id']} is  Paused.";
                        break;
                    case 3: $notification['title'] = "Offer ID : {$campaign['campaign_id']} is  Pending.";
                        break;
                    case 4: $notification['title'] = "Offer ID : {$campaign['campaign_id']} is  Deleted.";
                        break;
                    
                    default:
                        break;
                }
                $notification['link'] = SITEURL . "affiliate/campaign/offerRequest/{$campaign['campaign_id']}";
                $notification['description'] = "<a href='{$notification['link']}'><span class='noti_new_adv'>{$campaign['campaign_name']}</span></a>";
    ...
    public function setTargetting($campaign_id = 0) {
        //Update or set Geo Targeting and offer device ,offer OS
        $this->load->helper("form");
        $request = $this->input->post();
        $data = array();
        ...
        $data['FormAction'] = SITEURL . "admin/offer/setTargetting/" . $campaign_id;
        $data['SubmitBtn'] = "Save";
        $data['Submiting'] = "Saving...";
        $data['title'] = "Offer Targeting";
        $data['PageContent'] = $this->load->view("admin/offer/offer_edit/offer_geo_target", $data, TRUE);
        $this->load->view("admin/template", $data);
    }
