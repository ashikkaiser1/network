...
//        $data['offers'] = $this->m_campaign->getCampaign($filter);
        // $data['PageContent'] = $this->load->view("admin/dashboard/dashboard", $data, TRUE);
         $data['offers'] = array();
        $data['leaderboard'] = $this->load->view("admin/dashboard/v-leaderboard", null, TRUE);
        $data['PageContent'] = $this->load->view("admin/dashboard/dash_board_2", $data, TRUE);
...
//            $this->load->model("admin/m_reports");
//
//            $data['data'] = $this->m_reports->getTotalClicks()['clicks'];
//
//            echo json_encode($data);
//        }
    }
    ...
    public function getcommonStats($request = array()) {
        ...
        if ($request) {
            ...
            $request['select']['cost'] ='cost';
            $request['select']['revenue'] ='revemue';
            $request['select']['profit'] ='profit';
            $request['select']['conversion'] ='conversion';
            $request['select']['CR'] ='CR';
            $request['select']['clicks'] ='clicks';
            $request['all'] ='all';
            $data['data'] = $this->m_reports->getAdvanceReport($request);
            $data['success'] = TRUE;
            echo json_encode($data);
        }
        //  }
...
//    }
    ...
    public function getTotalUsers() {
        //show on wegits active and deactive
        $request = $this->input->post();
        if ($request) {
            $this->load->model("admin/m_reports");
            $row = $this->m_reports->getTotalUsers();
            $data['data'] = $row['active_user'] . "/" . $row['deactive_user'];
            echo json_encode($data);
        }
    }
    ...
    public function getCampaign() {
        ...
        if ($request) {
            $this->load->model("admin/m_reports");
            $row = $this->m_reports->getTotalCampaign();
            $data['data'] = $row['active_camp'] . "/" . $row['deactive_camp'];
            echo json_encode($data);
        }
    }
    ...
    public function getVisitors() {
        ...
        if ($request) {
            $this->load->model("admin/m_reports");
            $data['data'] = $this->m_reports->getTotalVisitors()['visitors'];
