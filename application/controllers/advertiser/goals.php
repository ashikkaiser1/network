...
//        $this->com->is_admin();
    ...
    public function CreateGoal($campaign_id = 0) {
        ...
        if ($request) {
            ...
            $request['payout_cost'] = isset($request['payout_cost']) ? $request['payout_cost'] : 0;
            if (defined('COMM_PERCENTAGE') && COMM_PERCENTAGE != 0) {
                $getAveraageRevenueCost = number_format((float) $request['revenue_cost'], 2, '.', '');
                $request['payout_cost'] = $getAveraageRevenueCost - (($getAveraageRevenueCost * COMM_PERCENTAGE) / 100);
            }
...
//        die();
        ...
        $filters['listFormated'] = TRUE;
        $data['affiliates'] = $this->m_users->getUsers($filters);
        $data['FormAction'] = SITEURL . "advertiser/goals/UpdateGoals/$campaign_id/$offer_goal_id";
        $data['SubmitBtn'] = "Create";
        $data['Submiting'] = "Creating...";
        ...
        // $data['Campaign'] = $this->m_campaign->getCategory(0,array("allchild"=>true));
        //$data['allCategory'] = $this->load->view("admin/Campaign/all-Campaign", $data, TRUE);
        $data['PageContent'] = $this->load->view("advertiser/goal/v-create-goal", $data, TRUE);
