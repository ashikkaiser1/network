...
//put your code here
    ...
    public function getRedirectOffer($offer = array()) {
        ...
        if (!empty($offer) && isset($offer['campaign_id'])) {
            ...
            $this->db->select("ofr.r_campaign_id,cmp.campaign_name")->from("offer_redirection ofr");
            $this->db->join("campaign cmp","cmp.campaign_id=ofr.campaign_id");
            $this->db->where("ofr.campaign_id", $offer['campaign_id']);
    ...
    public function addOfferCountry($campaign_id = 0, $countries = array()) {
        ...
        if (!empty($countries)) {
            foreach ($countries as $country) {
                if(is_array($country)){
                $insert_data[] = array("campaign_id" => $campaign_id,
                                "country_id" => $country['country_id'],
                                "payout_type" => $country['payout_type'],
                                "payout_cost" => $country['payout_cost']);    
...
//        $this->db->where("req_approval",0);
    ...
    public function getCampaign($filters = array()) {
        ...
        $select = ",GROUP_CONCAT(DISTINCT(cat.category_name)) as catName";
        $select.= ",GROUP_CONCAT(DISTINCT(countri.iso3)) as countryName";
        ...
        if (isset($filters['advertiser_id']) && is_array($filters['advertiser_id'])) {
            $this->db->where_in("c.advertiser_id", $filters['advertiser_id']);
        }
...
//  echo $this->db->last_query();
    ...
    public function setCampaignToCaps($campToCap = array(),$campaign_id=0) {
        //setting or insets the Daily Cap ,Daily Budget ,Monthly Cap ,Monthly Budget
        $this->delete_campaign_to_cap($campaign_id);
        if (!empty($campToCap)) {
    ...
    public function delete_campaign_to_cap($campaign_id) {
        
        $this->db->where("campaign_id",$campaign_id);
        $this->db->delete("campaign_to_cap");
    }
    ...
    //The functions are used for setting offer devices 
    //
    public function setOfferDevices($offer_devices = array(),$campaign_id=0) {
        $this->deleteOfferDevices($campaign_id);
        $this->db->insert_batch("offer_devices", $offer_devices);
        
    }
    
    public function getOfferDevices($filters= array()) {
        
        $this->db->select("*")->from("offer_devices");
        if(isset($filters['campaign_id']) && $filters['campaign_id'] !=''){
            $this->db->where("campaign_id", $filters['campaign_id']);
        }
    ...
    public function deleteOfferDevices($campaign_id) {
        //delete tehe offer countries
        $this->db->where("campaign_id", $campaign_id);
        $this->db->delete("offer_devices");
    }
    
    ///
    ///// end of offer device functionality
    
    
    //start . functionality offer OS setting 
    public function setOfferOS($offer_OSs = array(),$campaign_id=0) {
        $this->deleteOfferOS($campaign_id);
        $this->db->insert_batch("offer_os", $offer_OSs);
        
    }
    
    public function getOfferOS($filters= array()) { 
        
        $this->db->select("*")->from("offer_os");
        if(isset($filters['campaign_id']) && $filters['campaign_id'] !=''){
            $this->db->where("campaign_id", $filters['campaign_id']);
        }
    ...
    public function deleteOfferOS($campaign_id) {
        //delete tehe offer countries
        $this->db->where("campaign_id", $campaign_id);
        $this->db->delete("offer_os");
    }
    // end of set offer Operating System/ Platform
