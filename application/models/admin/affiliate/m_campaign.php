...
//        $this->db->where("req_approval",0);
    ...
    public function offerApprovedForUser($campaign_id = 0, $uid = 0, $filters = array()) {
        //check for is offer approved for user or not
...
//             $select =",uoap.uora_id as approved";
        ...
        $this->db->select("p.*,ctp.campaign_id,c.status,c.req_approval,c.payout_cost,ptype.name as payout_type, group_concat(cont.iso) as countries " . $select);
