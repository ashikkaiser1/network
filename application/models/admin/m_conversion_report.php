 ...
 * @author NexGen
 */
class m_conversion_report extends CI_Model {
...
//        if (isset($filters['select']['transaction_id'])) {
...
//        5 :CPC
        ...
        if (isset($filters['select']['cost'])) {
            $select.=",CASE
WHEN (ct.payout_type =4 OR ct.payout_type=8 OR ct.payout_type=11 OR ct.payout_type=12) THEN ROUND(sum(CASE WHEN trr.goal=0 THEN 1 ELSE 0 END) * ct.payout_cost ,2)
...
//ROUND(sum(ct.payout_cost),2) as Payout1 
        ...
        if (isset($filters['select']['revenue'])) {
            $select.=",CASE 
WHEN (ct.payout_type =4 OR ct.payout_type=8 OR ct.payout_type=11 OR ct.payout_type=12) THEN ROUND(sum(CASE WHEN trr.goal=0 THEN 1 ELSE 0 END) * cmp.revenue_cost ,2)
...
//ROUND((SELECT Revenu) - (SELECT Payout)  ,2)
        if (isset($filters['select']['profit'])) {
            ...
            $select.=",CASE 
WHEN (ct.payout_type =4 OR ct.payout_type=8 OR ct.payout_type=11 OR ct.payout_type=12) THEN ROUND(($qry2 - $qry1),2)
WHEN ct.payout_type=5 THEN ROUND(sum(cmp.revenue_cost) - sum(ct.payout_cost),2)
ELSE '0'
END as Profit";
        }
//
//}//earned by admin
        ...
        $this->db_reader->join("transactions trr", "trr.transaction_id=ct.transaction_id", "RIGHT");
      //  if (isset($filters['goals'])) {
...
//        }
        ...
        if (isset($filters['transaction_id']) && $filters['transaction_id'] != '') {
            $this->db_reader->where_in("ct.transaction_id", $filters['transaction_id']);
        }
...
//        ct.clicktracker_id,
        ...
        if (isset($filters['groupby']['click_valid'])) {
            $this->db_reader->where("ct.valid", 0);
        }
        else
        {
    ...
    private function group_by_return($group_by_array) {
        ...
        if (!isset($group_by_array['groupby']))
            return 'ct.transaction_id,';
        $group_by = 'ct.transaction_id,';
        $select = '';
...
//                $this->db_reader->group_by("ct.uid");
            ...
            if (isset($val) && $val == 'transaction_id') {
                $group_by.="ct.transaction_id,";
                $select.= ",ct.transaction_id";
            }

            if (isset($val) && $val == 'c_valid') {

                $select.= ",(CASE WHEN (trr.c_valid=1 AND ct.valid=1) THEN 1 ELSE 0 END ) as Conv_Status";
            }
    ...
    public function ChangeStatus($filters = array(), $updateData = array()) {
        if (isset($filters['transaction_id']) && $filters['transaction_id'] != '' && !is_array($filters['transaction_id'])) {
            $this->db->where("transaction_id", $filters['transaction_id']);
            $this->db->update("transactions", $updateData);
            return TRUE;
            //$this->db->affected_rows();
        }

        if (isset($filters['transaction_id']) && is_array($filters['transaction_id'])) {

            $this->db->where_in("transaction_id", $filters['transaction_id']);
            $this->db->update("transactions", $updateData);
...
//    ChangeClickStatus
     ...
     public function ChangeClickStatus($filters = array(), $updateData = array()) {
        if (isset($filters['transaction_id']) && $filters['transaction_id'] != '' && !is_array($filters['transaction_id'])) {
            $this->db->where("transaction_id", $filters['transaction_id']);
