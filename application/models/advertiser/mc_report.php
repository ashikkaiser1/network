 ...
 * @author NexGen
 */
class mc_report extends CI_Model {
...
//        //}
        ...
        if (isset($filters['select']['clicks'])) {
          $select.=",  {$this->clickFormula} as Clicks ";
            //$select.=",  sum(DISTINCT CASE WHEN ct.track_type=0 and ct.transaction_id  THEN 1 ELSE 0 END) as Clicks ";
        }
        ...
        if (isset($filters['select']['CR'])) {
                $select.=", ROUND( (sum(CASE WHEN (trr.goal=0 and trr.c_valid=1)  THEN 1 ELSE 0 END) / sum(CASE WHEN (ct.track_type=0 and (trr.goal=0 or trr.goal IS NULL)) THEN 1 ELSE 0 END)) * 100,2 ) as CR   ";
        }
...
//WHEN '4' THEN ROUND(sum(CASE WHEN (trr.goal=0 and trr.c_valid=1)  THEN 1 ELSE 0 END) * ct.payout_cost ,2)
...
// Advertiser payout is the revenue of admin so advertiser Payout = admin Revenue
        if (isset($filters['select']['payout'])) {
            $select.=",CASE 
WHEN (ct.payout_type =4 OR ct.payout_type=8 OR ct.payout_type=11 OR ct.payout_type=12) THEN ROUND(sum(CASE WHEN (trr.goal=0 and trr.c_valid=1)  THEN 1 ELSE 0 END) * cmp.revenue_cost ,2)
WHEN ct.payout_type=5 THEN ROUND(sum(cmp.revenue_cost),2)
ELSE '0'
END as Payout";
        }
...
//ROUND((SELECT Revenu) - (SELECT Payout)  ,2)
        if (isset($filters['select']['profit'])) {
            $qry1 = "sum(CASE WHEN (trr.goal=0 and trr.c_valid=1)  THEN 1 ELSE 0 END ) * ct.payout_cost";
            $qry2 = "sum(CASE WHEN (trr.goal=0 and trr.c_valid=1)  THEN 1 ELSE 0 END) * cmp.revenue_cost ";
            $select.=",CASE ct.payout_type 
...
//        }
        ...
        if (isset($filters['uid']) && $filters['uid'] != '') {
            $this->db_reader->where_in("advertiser.uid", $filters['uid']);
        }
...
//        $this->db_reader->where("");
        ...
        if (isset($filters['orderby']) && $filters['orderby'] == 'clicks') {
            $sort = '';
            if (isset($filters['sort']) && $filters['sort'] != '') {
                $sort = $filters['sort'];
            }

            $this->db_reader->order_by("Clicks", $sort);
        }
        ...
        if (isset($filters['orderby']) && $filters['orderby'] == 'payout') {
            $sort = '';
            if (isset($filters['sort']) && $filters['sort'] != '') {
                $sort = $filters['sort'];
            }

            $this->db_reader->order_by("Payout", $sort);
        }
...
//        ct.clicktracker_id,
       ...
       // $this->db_reader->where("trr.c_valid", 1);
        ...
        if (isset($filters['limit']) && $filters['limit'] != '') {
            ...
            $this->db_reader->limit($offset, (int) $filters['limit']);
        } else {
...
//                $group_by.=",u.uid";
            ...
            if (isset($val) && $val == 'goal_name') {
                // $this->db_reader->group_by("ct.campaign_id");
                $group_by.="trr.goal,";
                $select.=",offg.name as Goal_Name ,sum(CASE WHEN trr.goal=offg.offer_goal_id and offg.campaign_id = ct.campaign_id THEN 1 ELSE 0 END) as Total_Unit";
            }
...
//                $this->db_reader->group_by("ct.uid");
    ...
    public function getReport($filters = array()) {
        ...
        $this->db_reader->select("sum(CASE WHEN (ct.track_type=0 and (trr.goal=0 or trr.goal IS NULL)) THEN 1 ELSE 0 END) as clicks ,ROUND(sum(CASE WHEN (trr.goal >=0 and trr.c_valid=1) THEN 1 ELSE 0 END) * ct.payout_cost ,2) as earn,Date(ct.dateTime) as date $select", FALSE)->from("click_tracker ct");
//        $this->db_reader->where("");
...
//        }
    ...
    public function getstats_old($filters = array()) {
...
//WHEN '4' THEN (CASE WHEN (trr.goal=0 and trr.c_valid=1) THEN 1 ELSE 0 END) * ct.payout_cost 
//WHEN '5' THEN  ct.payout_cost
//ELSE '0'
//END),2) as Cost";
        
        $select.=",CASE ct.payout_type 
WHEN '4' THEN ROUND(sum(CASE WHEN (trr.goal=0 and trr.c_valid=1)  THEN 1 ELSE 0 END) * cmp.revenue_cost ,2)
...
//ROUND(sum(ct.payout_cost),2) as Payout1 
//        $select.=",ROUND(SUM(CASE  ct.payout_type 
//WHEN '4' THEN (CASE WHEN (trr.goal=0 and trr.c_valid=1)  THEN 1 ELSE 0 END) * cmp.revenue_cost
...
//      
        ...
        if (isset($filters['uid']) && $filters['uid'] != '') {
//            $this->db_reader->where("ct.uid", $filters['uid']);
             $this->db_reader->where("advertiser.uid", $filters['uid']);
        }
