...
class m_links extends CI_Model {
    ...
    public function getLinks($filters = array()) {
        $link = array();
        $this->db->select("p.title,lk.gen_link,lk.post_back,lk.link_id")->from("link lk");
    ...
    public function getLinkEvents($filter = array()) {
        $this->db->select("og.name as eventName,og.offer_goal_id,og.offer_goal_id,le.callback");
        $this->db->from("offer_goal og");
        $this->db->join("link_event_post le", "le.offer_goal_id=og.offer_goal_id and le.link_id = {$filter['link_id']}", "LEFT");
