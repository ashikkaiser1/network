...
//             $select =",uoap.uora_id as approved";
        ...
        $select = ",uoapp.status as approve_offer, $aff_payout ,$aff_group_payout";
        $this->db->select("p.*,ctp.campaign_id,c.status,c.req_approval,c.payout_cost,ptype.name as payout_type, group_concat(DISTINCT(cont.iso)) as countries, group_concat(DISTINCT(categ.category_name)) as catName " . $select);
...
//        , group_concat(cont.iso separator ',') as countries" 
    ...
    public function get_custom_payouts($filters = array()) {

        $this->db->select("pg.payout_cost , pt.name as ptypeName, pg.payout_type")->from("pay_group pg");
        $this->db->join("pay_type as pt", "pt.pay_type_id=pg.payout_type", "LEFT");

        if (isset($filters['uid']) && $filters['uid'] != '') {
            $this->db->where("pg.uid", $filters['uid']);
        }
        if (isset($filters['campaign_id']) && $filters['campaign_id'] != '') {
    ...
    public function get_group_payout($filters = array()) {
        $this->db->select("pg.payout_cost , pt.name as ptypeName, pg.payout_type")->from("pay_group pg");
        $this->db->join("pay_type as pt", "pt.pay_type_id=pg.payout_type", "LEFT");
        $this->db->join("usr_group_details as ugd", "ugd.group_id=pg.group_id", "LEFT");

        if (isset($filters['uid']) && $filters['uid'] != '') {
            $this->db->where("ugd.uid", $filters['uid']);
        }
        if (isset($filters['campaign_id']) && $filters['campaign_id'] != '') {
            $this->db->where("pg.campaign_id", $filters['campaign_id']);
        }
