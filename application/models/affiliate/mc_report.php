...
//        5 :CPC
        ...
        if (isset($filters['select']['payout'])) {
            $select .= ",CASE 
WHEN (ct.payout_type =4 OR ct.payout_type=8 OR ct.payout_type=11 OR ct.payout_type=12) THEN ROUND(sum(CASE WHEN (trr.goal=0 and trr.c_valid=1)  THEN 1 ELSE 0 END) * ct.payout_cost ,2)
WHEN ct.payout_type=5 THEN ROUND(count(ct.link_id)*ct.payout_cost,2)
ELSE '0'
END as Payout";
            //Payout
        }
